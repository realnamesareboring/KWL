...
# FIXED: Use consistent log file path that persists across reboots
$global:LogFileDir = $PSScriptRoot
if (-not $global:LogFileDir) { $global:LogFileDir = "C:\Projects\KWLTool" }
$global:LogFile = Join-Path $global:LogFileDir "KWL-AutoDeploy-$(Get-Date -Format 'yyyyMMdd').log"
...
function Write-KWLLog {
    ...
    # FIXED: Ensure log directory exists and handle file locking
    try {
        ...
        # Fallback to temp directory if main log fails
        $fallbackLog = "$env:TEMP\KWL-AutoDeploy-Fallback.log"
...
function Show-KWLBanner {
    ...
    Write-Host "    WITH INTEGRATED DOCKER PROFILE BYPASS v3.0" -ForegroundColor Cyan
    Write-Host ""
...
function Save-Progress {
    ...
    $progress = @{
        ...
        LogFile = $global:LogFile  # FIXED: Preserve log file path across reboots
        Parameters = @{
...
function Get-Progress {
    if (Test-Path $global:ProgressFile) {
        try {
            ...
            # FIXED: Restore log file path from progress
            if ($progress.LogFile) {
                $global:LogFile = $progress.LogFile
                Write-KWLLog "Restored log file path: $global:LogFile" 'INFO' 'Gray'
            }
...
# NEW FUNCTION: Test Docker Engine Health
function Test-DockerEngineHealth {
    param([int]$TimeoutMinutes = 10)
    
    Write-KWLLog "Testing Docker Engine health..." 'INFO' 'Yellow'
    ...
    while ((Get-Date) -lt $timeout) {
        try {
            # Test 1: Docker info command
            $dockerInfo = docker info --format "{{.ServerVersion}}" 2>$null
            $infoHealthy = ($LASTEXITCODE -eq 0 -and $dockerInfo)
            
            # Test 2: Docker system ping  
            $dockerPing = docker system ping 2>$null
            $pingHealthy = ($LASTEXITCODE -eq 0)
            
            # Test 3: Docker version check
            $dockerVersion = docker version --format "{{.Server.Version}}" 2>$null
            $versionHealthy = ($LASTEXITCODE -eq 0 -and $dockerVersion)
            
            $allHealthy = $infoHealthy -and $pingHealthy -and $versionHealthy
            
            if ($allHealthy) {
                Write-KWLLog "Docker Engine health check: PASSED" 'SUCCESS' 'Green'
                Write-KWLLog "  - Docker Info: OK (Version: $dockerInfo)" 'INFO' 'Green'
                Write-KWLLog "  - Docker Ping: OK" 'INFO' 'Green' 
                Write-KWLLog "  - Docker Version: OK (Server: $dockerVersion)" 'INFO' 'Green'
                return $true
            }
            else {
                $healthyStates += $allHealthy
    ...
    Write-Host ""
    Write-KWLLog "Docker Engine health check: FAILED after $TimeoutMinutes minutes" 'ERROR' 'Red'
    Write-KWLLog "  - Docker Info: $(if($infoHealthy){'OK'}else{'FAILED'})" 'ERROR' 'Red'
    Write-KWLLog "  - Docker Ping: $(if($pingHealthy){'OK'}else{'FAILED'})" 'ERROR' 'Red'
    Write-KWLLog "  - Docker Version: $(if($versionHealthy){'OK'}else{'FAILED'})" 'ERROR' 'Red'
    return $false
}
...
# UPDATED FUNCTION: Create Docker Profile Bypass Settings
function Set-DockerProfileBypass {
    ...
    try {
        ...
        # Step 3: Apply registry settings for Docker bypass
        ...
        # Step 4: FIXED - Use correct registry path for auto-start
        Write-KWLLog "Configuring Docker auto-start in Windows startup..." 'INFO' 'Cyan'
        $runKey = "HKCU:\Software\Microsoft\Windows\CurrentVersion\Run"
        $dockerExePath = "C:\Program Files\Docker\Docker\Docker Desktop.exe"
        
        # Set the exact registry value that Docker uses
        try {
            Set-ItemProperty -Path $runKey -Name "Docker Desktop" -Value $dockerExePath -ErrorAction Stop
            Write-KWLLog "FIXED: Set Docker auto-start registry key to: $dockerExePath" 'SUCCESS' 'Green'
            
            # Verify the setting was applied
            $verifyValue = Get-ItemProperty -Path $runKey -Name "Docker Desktop" -ErrorAction SilentlyContinue
            if ($verifyValue -and $verifyValue."Docker Desktop" -eq $dockerExePath) {
                Write-KWLLog "Verified: Docker auto-start registry setting is correct" 'SUCCESS' 'Green'
            }
            else {
                Write-KWLLog "Warning: Could not verify Docker auto-start registry setting" 'WARN' 'Yellow'
            }
        }
        catch {
            Write-KWLLog "Failed to set Docker auto-start registry: $($_.Exception.Message)" 'ERROR' 'Red'
        }
...
# NEW FUNCTION: Launch Docker Dashboard and close it (the workaround)
function Start-DockerDashboardOnce {
    Write-KWLLog "Launching Docker Dashboard for first-time setup completion..." 'INFO' 'Cyan'
    
    try {
        $dockerExePath = "C:\Program Files\Docker\Docker\Docker Desktop.exe"
        
        if (-not (Test-Path $dockerExePath)) {
            Write-KWLLog "Docker Desktop executable not found at: $dockerExePath" 'ERROR' 'Red'
            return $false
        }
        
        # Step 1: Launch Docker Desktop
        Write-KWLLog "Starting Docker Desktop..." 'INFO' 'Yellow'
        $dockerProcess = Start-Process -FilePath $dockerExePath -PassThru -WindowStyle Normal
        Write-KWLLog "Docker Desktop process started (PID: $($dockerProcess.Id))" 'SUCCESS' 'Green'
        
        # Step 2: Wait for Docker to initialize and show dashboard
        Write-KWLLog "Waiting for Docker Dashboard to appear..." 'INFO' 'Yellow'
        Start-Sleep -Seconds 45  # Give Docker time to fully load
        
        # Step 3: Check if Docker engine is healthy
        Write-KWLLog "Checking if Docker engine is responding..." 'INFO' 'Yellow'
        $engineHealthy = Test-DockerEngineHealth -TimeoutMinutes 5
        
        if ($engineHealthy) {
            Write-KWLLog "Docker engine is healthy, dashboard should have appeared" 'SUCCESS' 'Green'
        }
        else {
            Write-KWLLog "Docker engine not fully healthy yet, but continuing..." 'WARN' 'Yellow'
        }
        
        # Step 4: Close Docker Desktop window (not the tray)
        Write-KWLLog "Closing Docker Desktop window (keeping tray service running)..." 'INFO' 'Yellow'
        try {
            # Find Docker Desktop windows and close them
            $dockerWindows = Get-Process | Where-Object { $_.ProcessName -eq "Docker Desktop" -and $_.MainWindowTitle }
            
            if ($dockerWindows) {
                foreach ($window in $dockerWindows) {
                    if ($window.CloseMainWindow()) {
                        Write-KWLLog "Closed Docker Desktop window (PID: $($window.Id))" 'SUCCESS' 'Green'
                    }
                }
            }
            else {
                Write-KWLLog "No Docker Desktop windows found to close (may already be minimized)" 'INFO' 'Yellow'
            }
            
            # Alternative: Send Alt+F4 to any Docker Desktop windows
            ...
            Write-KWLLog "Error closing Docker window: $($_.Exception.Message)" 'WARN' 'Yellow'
        }
        
        # Step 5: Verify Docker is still running in background
        Start-Sleep -Seconds 10
        $dockerStillRunning = docker info 2>$null
        if ($LASTEXITCODE -eq 0) {
            Write-KWLLog "SUCCESS: Docker Dashboard closed, but engine still running in background" 'SUCCESS' 'Green'
            return $true
        }
        else {
            Write-KWLLog "Warning: Docker engine may not be running after closing dashboard" 'WARN' 'Yellow'
            return $false
        }
    }
    catch {
        Write-KWLLog "Failed to launch/close Docker Dashboard: $($_.Exception.Message)" 'ERROR' 'Red'
        return $false
    }
}
...
# UPDATED: Enhanced Deploy-KustoEmulator function with health checks
function Deploy-KustoEmulator {
    Write-KWLLog "Deploying Kusto Emulator container..." 'INFO' 'Yellow'
    
    try {
        # FIXED: Ensure Docker Engine is healthy before proceeding
        Write-KWLLog "Verifying Docker Engine health before Kusto deployment..." 'INFO' 'Cyan'
        if (-not (Test-DockerEngineHealth -TimeoutMinutes 5)) {
            Write-KWLLog "Docker Engine is not healthy, cannot deploy Kusto" 'ERROR' 'Red'
            return $false
        }
        ...
        # FIXED: Ensure Kusto image is downloaded with health verification
        Write-KWLLog "Checking for Kusto Emulator image..." 'INFO' 'Cyan'
        $imageName = "mcr.microsoft.com/azuredataexplorer/kustainer-linux:latest"
        $existingImage = docker images --filter "reference=$imageName" --format "{{.Repository}}:{{.Tag}}" 2>$null
        
        if ($existingImage -eq $imageName) {
            Write-KWLLog "Kusto Emulator image already exists" 'SUCCESS' 'Green'
        }
        else {
            Write-KWLLog "Downloading Kusto Emulator image (this may take several minutes)..." 'INFO' 'Cyan'
            Write-KWLLog "Docker Engine verified as healthy, starting image pull..." 'INFO' 'Cyan'
            
            $pullOutput = docker pull $imageName 2>&1
            
            if ($LASTEXITCODE -eq 0) {
                Write-KWLLog "Successfully downloaded Kusto Emulator image" 'SUCCESS' 'Green'
                
                # Verify image was actually downloaded
                $verifyImage = docker images --filter "reference=$imageName" --format "{{.Repository}}:{{.Tag}}" 2>$null
                if ($verifyImage -eq $imageName) {
                    Write-KWLLog "Verified: Kusto image is available locally" 'SUCCESS' 'Green'
                }
                else {
                    Write-KWLLog "Warning: Image pull reported success but image not found locally" 'WARN' 'Yellow'
                }
            }
            else {
                Write-KWLLog "Pull output: $pullOutput" 'ERROR' 'Red'
                throw "Failed to pull Kusto Emulator image. Exit code: $LASTEXITCODE"
            }
        }
        ...
        $containerOutput = docker @dockerCommand 2>&1
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            if ($runningContainer -eq "kusto-emulator") {
                ...
                $containerLogs = docker logs kusto-emulator 2>&1
                Write-KWLLog "Container logs: $containerLogs" 'DEBUG' 'Gray'
                return $false
            }
        }
        else {
            Write-KWLLog "Docker run output: $containerOutput" 'ERROR' 'Red'
            throw "Failed to start Kusto Emulator container. Exit code: $LASTEXITCODE"
        }
    }
    catch {
...
function Start-KWLAutoDeploy {
    ...
    Write-KWLLog "Starting KWL Auto-Deploy process with integrated Docker bypass v3.0..." 'INFO' 'Cyan'
    ...
    # FIXED: Restore log file path from progress before other operations
    ...
    switch ($currentPhase) {
        ...
        "DOCKER_WAIT" {
            Write-KWLLog "=== PHASE 3: DOCKER SETUP + DASHBOARD LAUNCH ===" 'INFO' 'Magenta'
            Save-Progress -Phase "DOCKER_WAIT"
            
            # INTEGRATION POINT 3: Re-apply bypass settings
            ...
            # NEW: Launch Docker Dashboard once and close it (the workaround)
            Write-KWLLog "Performing Docker Dashboard workaround..." 'INFO' 'Cyan'
            Start-DockerDashboardOnce
        }
    ...
    if ($currentPhase -ne "COMPLETE") {
        ...
        # INTEGRATION POINT 4: Final verification
        Write-KWLLog "Verifying Docker bypass and auto-start settings..." 'INFO' 'Cyan'
        ...
        # Verify auto-start registry setting
        $runKey = "HKCU:\Software\Microsoft\Windows\CurrentVersion\Run"
        $autoStartValue = Get-ItemProperty -Path $runKey -Name "Docker Desktop" -ErrorAction SilentlyContinue
        if ($autoStartValue -and $autoStartValue."Docker Desktop") {
            Write-KWLLog "Docker auto-start registry setting confirmed: $($autoStartValue.'Docker Desktop')" 'SUCCESS' 'Green'
        }
        else {
            Write-KWLLog "Warning: Docker auto-start registry setting not found" 'WARN' 'Yellow'
        }
...
function Show-PostInstallInstructions {
    ...
    Write-Host "   Image: Downloaded and running" -ForegroundColor Green
    Write-Host ""
    ...
    Write-Host "   Dashboard workaround: APPLIED" -ForegroundColor Green
    Write-Host "   Auto-start: ENABLED (Registry path: HKCU\...\Run)" -ForegroundColor Green
    Write-Host "   User group: Added to docker-users" -ForegroundColor Green
    Write-Host "   Engine health: VERIFIED" -ForegroundColor Green
    Write-Host ""
    ...
    Write-Host "   - Starts automatically with Windows login" -ForegroundColor Green
    Write-Host "   - Dashboard launched once to complete setup" -ForegroundColor Green
    Write-Host "   - NO sign-in prompts on future startups" -ForegroundColor Green
    ...
    Write-Host "   Persistent across reboots" -ForegroundColor Green
    Write-Host ""
    ...
    Write-Host "   [SUCCESS] Docker Dashboard launched once and closed (workaround)" -ForegroundColor Green
    Write-Host "   [SUCCESS] Auto-start enabled via correct registry path" -ForegroundColor Green
    Write-Host "   [SUCCESS] Docker Engine health verified before Kusto deployment" -ForegroundColor Green
    Write-Host "   [SUCCESS] Kusto image downloaded and container running" -ForegroundColor Green
    Write-Host "   [SUCCESS] Logging persistent across reboots" -ForegroundColor Green
    Write-Host ""
}
